...
def push(channel_name, data):
    ...
    for channel in data:
        ...
        if 'end_time' in channel:
            stop = 'stop="{} +0300"'.format(datetime.fromtimestamp(channel['end_time'], pytz.timezone('Europe/Athens')).strftime('%Y%m%d%H%M%S'))

        append('<programme start="{} +0300" {} channel="{}">'.format(start, stop, channel_name))
